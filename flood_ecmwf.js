...
function load_flood_ecmwf(error, array) {
/*
 ...
 */
	...
	for (let index in array) {
		draw_flood_ecmwf(array[index], index);
	}
}

function draw_flood_ecmwf(dataArray, index) {
/*
...
*/
	...
	const canvas = svg_map_flood_ecmwf 
		...
		.attr("stroke-width", 0)
		.append('xhtml:canvas')
			.attr('id', 'canvas_flood_ecmwf_viz_' + index)
			.attr("width", w)
			.attr("height", h)
			.style('display', 'none')
	...
	// Draw canvas
	drawCanvasFloodEcmwf(dataContainer, context, canvas);
}

function drawCanvasFloodEcmwf(dataContainer, context, canvas) {
/*
 ...
 */
	...
	elements.each(function(d) {
		...
		context.beginPath();
		context.fillStyle = colorScaleFlood(val);
		// rect(x,y,width,height)
		context.rect(coords[0], coords[1], 5, 5);
		context.fill();
		context.closePath();
	})
}
